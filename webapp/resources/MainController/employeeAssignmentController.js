app.controller('authoriCon',['$scope','$http',function($scope, $http){	
				...
				$scope.pageSizeAuthGN = {};
				$scope.pageSizeAuthGN.rows = [ 
								{ value: "5", label: "5" },
			    				{ value: "10", label: "10" },
			            		{ value: "15", label: "15" },
			            		{ value: "20", label: "20" },
				...
				$scope.pageSizeAuthGN.row = $scope.pageSizeAuthGN.rows[1].value;
				...
				$scope.changeProcess = function(){
					...
					if($scope.modelApp != ""){
						...
						$http({
				 			method: 'GET',
						    url: server+"rest/authorizationemployee/getprocess/"+$scope.modelApp,
						    headers: {
						...
						$http({
				 			method: 'GET',
						    url: server+"rest/authorizationemployee/getprocess/ame",
						    headers: {
						    	'Accept': 'application/json',
						        'Content-Type': 'application/json'
						    } 
						...
						if(Object.keys($scope.listEmpDetail).length != 0){
					        ...
					        function(isConfirm){ 
					            ...
					            if(isConfirm){
									...
									$http({
							 			method: 'POST',
									    url: server+"rest/authorizationemployee/create",
									    headers: {
									    	'Accept': 'application/json',
									        'Content-Type': 'application/json'
									    }	,
							...
							swal({
								  title: "Information",
								  text: "Please assignment employee to authorization !.",
								  timer: 3000,
								  type: "info",
				...
				$scope.listProcess = function(){
					$http({
			 			method: 'GET',
					    url: server+"rest/authorizationemployee/getprocess",
					    headers: {
					    	'Accept': 'application/json',
					        'Content-Type': 'application/json'
					    }	    
				...
				$scope.getSubAdd = function(){
					...
					if($("#process").val() != ""){
						$http({
				 			method: 'GET',
						    url: server+"rest/authorizationemployee/getSubAdd/"+$scope.processMod,
						    headers: {
						    	'Accept': 'application/json',
						        'Content-Type': 'application/json'
						    }	    
				...
				$scope.deleteAuthEmployeeById = function(empId,process,authID){
					...
					swal({
						title:  "<span style='font-size: 20px;'>You are about to delete process with "+process+".</span>",
						text: "Click OK to continue or CANCEL to abort.",
						type: "info",
						html: true,
						showCancelButton: true,
			        ...
			        function(isConfirm){ 
			            ...
			            if(isConfirm){
			            	setTimeout(function(){
					            	...
					            	$http({   
					            		method:"POST",  
									    url: server+"rest/authorizationemployee/deleteById/"+empId+"/"+process+"/"+authID,
									    headers: {
									    	'Accept': 'application/json',
									        'Content-Type': 'application/json'
									    }	    
